...
export const PostList = () => {
    ...
    return (<>
        <section className="Section__Post">
            <table>
                <tr>
                    <th className="Post_Item"></th>
                    <th className="Post_Item">Item</th>
                    <th className="Post_Item">Settlement</th>
                    <th className="Post_Item">Server</th>
                </tr>
                {
                    Posts.map(Post => {
                        let Itemrarity;
                        ...
                        return (<>
                            <tr className="Post" id={`Post--${Post.id}`} key={`${Post.id}`}>
                                <td className="Post_image"><Link to={`/posts/${Post.id}`}><div style={itemRaritybackground} className="Post_image_background">
                                    <img className="Post_item_image" src={Post.item?.image} alt="...Loading" /></div></Link></td>
                                <td className="post_td"><Link className="Post_Item" to={`/posts/${Post.id}`}>{Post.item?.itemName}</Link></td>
                                <td className="post_td"><Link className="Post_Item" to={`/posts/${Post.id}`}>{Post.settlementId?.settlementName}</Link></td>
                                <td className="post_td"><Link className="Post_Item" to={`/posts/${Post.id}`}>{Post.posterId?.server.serverName}</Link></td>
                            </tr>
                        </>)
                    })
                }
